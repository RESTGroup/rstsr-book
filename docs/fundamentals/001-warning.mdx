...
- **RSTSR does not support automatic differentiation**. By design, RSTSR follows ndarray's strategy, implementing different ownerships (Owned, View, Mut, Cow, Arc, Reference types, etc.) differently. Most operations return Owned tensors. This approach is not suitable for automatic differentiation programs, which generally need to record backpropagation information and are better implemented with Arc or RwLock. Due to this design pattern, RSTSR has decided not to support automatic differentiation. **This does not mean that RSTSR cannot implement machine learning inference computations in the future, but machine learning operators are currently not a primary supported feature**. RSTSR's focus will lean more towards linear algebra and numerical algebra.
