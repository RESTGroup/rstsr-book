...
```rust file=indexing anchor=example_index_by_range_01
```

First two dimensions slicing are also available by the following way:

```rust file=indexing anchor=example_index_by_range_02
```

Negative indices are also applicable for this case:

```rust file=indexing anchor=example_index_by_range_03
```
...
```rust file=indexing anchor=example_index_by_range_04
```
...
```rust file=indexing anchor=example_index_by_range_05
```
...
```rust file=indexing anchor=example_slice_with_strides_01
```
...
```rust file=indexing anchor=example_slice_with_strides_02
```
...
```rust file=indexing anchor=example_insert_axes_01
```
...
```rust file=indexing anchor=example_insert_axes_02 ferris=panics
```
...
```rust file=indexing anchor=example_ellipsis_01
```
...
```rust file=indexing anchor=example_mixed_indexing
```
...
### Safe elementwise indexing

To perform indexing, you may use rust's bracket `[]`:

```rust file=indexing anchor=example_elementwise_safe
```

If you provides index out-of-bound, RSTSR will panic:

```rust file=indexing anchor=example_elementwise_safe_panic ferris=panics
```
...
```rust file=indexing anchor=example_elementwise_by_tensor_index
```

### Unchecked elementwise indexing
...
```rust file=indexing anchor=example_elementwise_unchecked
```
...
```rust file=indexing anchor=example_elementwise_unchecked_not_desired ferris=not_desired_behavior
```
