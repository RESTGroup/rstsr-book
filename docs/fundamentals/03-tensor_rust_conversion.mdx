...
```rust file=tensor_rust_conversion anchor=example_from_vec_by_asarray
```

### From `Vec<T>`: Manual Construction
...
```rust file=tensor_rust_conversion anchor=example_from_vec_by_scratch
```

### Into `Vec<T>`: into_vec Function
...
- This function prohibits converting higher-dimensional tensors (e.g., 2-D) into vectors:
        ...
        ```rust file=tensor_rust_conversion anchor=example_into_vec_by_into_vec_failed ferris=does_not_compile
    ```
...
- For `Tensor<T, B, D>` (tensors that own their data), this function typically does not copy data, meaning it has almost no runtime cost:
    ...
    ```rust file=tensor_rust_conversion anchor=example_into_vec_by_into_vec
    ```
    ...
    ```rust file=tensor_rust_conversion anchor=example_into_vec_by_into_vec_cloned
    ```

- For reference types (e.g., `TensorView<'_, T, B, D>`), this function will copy data.

### Into `Vec<T>`: Top-down Deconstruction
...
```rust file=tensor_rust_conversion anchor=example_into_vec_destruct
```
...
```rust file=tensor_rust_conversion anchor=example_into_vec_destruct_warn
```
...
## Conversions with `&[T]`/`&mut [T]` or Pointer Types
...
```rust file=tensor_rust_conversion anchor=example_from_ref_by_scratch
```
...
### To `&Vec<T>`: raw Function

We can return a reference to the underlying data:

```rust file=tensor_rust_conversion anchor=example_to_ref_by_raw
```
...
```rust file=tensor_rust_conversion anchor=example_raw_error_usage_1
```
...
```rust file=tensor_rust_conversion anchor=example_raw_error_usage_2
```
...
```rust file=tensor_rust_conversion anchor=example_raw_error_usage_3 ferris=not_desired_behavior
```
...
```rust file=tensor_rust_conversion anchor=example_raw_error_usage_4
```

:::

## Conversions with Faer Types
...
```rust file=tensor_rust_conversion anchor=example_rstsr_faer_conversion
```