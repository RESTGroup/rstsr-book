...
## Converting Rust Vector to RSTSR Tensor

### 1-D tensor from rust vector
...
```rust file=tensor_creation anchor=example_01
```
...
[^1]:
    ...
    ```rust file=tensor_creation anchor=example_02
    ```

### $n$-D tensor from rust vector
...
```rust file=tensor_creation anchor=example_03
```
...
```rust file=tensor_creation anchor=example_04
```

## Converting Rust Slices to RSTSR TensorView
...
## Intrinsic RSTSR Tensor Creation Functions

### 1-D tensor creation functions
...
```rust file=tensor_creation anchor=example_arange
```
...
```rust file=tensor_creation anchor=example_linspace
```

### 2-D tensor creation functions
...
```rust file=tensor_creation anchor=example_eye
```
...
```rust file=tensor_creation anchor=example_diag
```

### General $n$-D tensor creation functions
...
```rust file=tensor_creation anchor=example_zeros_01
```
...
```rust file=tensor_creation anchor=example_zeros_02
```
...
```rust file=tensor_creation anchor=example_zeros_03
```

You may also initialize a tensor without filling specific values. This is unsafe.

```rust file=tensor_creation anchor=example_empty
```
...
```rust file=tensor_creation anchor=example_random
```
