...
In this mean time, you can also directly call use `rstsr` crate, which is a wrapper to `rstsr_core`:
```rust
use rstsr::prelude::*;
```
...
```rust file=../../listings/features-default/tests/tensor_creation.rs start=ANCHOR:example_01 end=ANCHOR_END:example_01
```
<img src="/img/ferris/panics.svg" alt="panic" />
</div> -->
